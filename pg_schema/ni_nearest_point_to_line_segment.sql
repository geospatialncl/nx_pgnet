--issues to address:
	--column structure for returning records
	--what happens when the link length is less than the desired length of line?
	--cleaning up of code base
	--need to check outputs of join to line and point tables are correct

--function to help determine most likely substation providing power to the rail network

--select all sections of electrified rail or assume that the user is supplying a section of rail as the input that is electrified
--$1 - supply the line dataset table name
--$2 - supply the line dataset geometry column name
--$3 - supply the line dataset prkey
--$4 - prefix for line data to be applied to joined output (to avoid duplicate column names in joined outputs)
--$5 - supply the point dataset table name
--$6 - supply the point dataset geometry column name
--$7 - supply the point dataset prkey
--$8 - prefix for point data to be applied to joined output (to avoid duplicate column names in joined outputs)
--$9 - supply the likely substation separation distance for railway electrification supply e.g. 500m, 2000m etc. = sub_sep_distance
--$10 - supply the initial search distance for a buffer around the line segment buff_distance
--$11 - output table name
--$12 - boolean to denote whether or not to add the resultant table to the geometry columns table

--e.g. SELECT * FROM ni_nearest_point_to_line_segment('os_meridian_2_rail_ln_polyline', 'geom', 'code', 'linedata_', 'OS_ElectricitySubStations', 'geom', 'gid', 'pointdata_', 2000, 500, 'closest_os_electricitysubstation_to_electrified_rail_sections', false) f(point_prkey integer, point_geom geometry, point_fraction double precision, perp_point_on_line geometry, point_to_perp_point_line geometry, point_to_perp_point_line_length double precision, line_prkey integer, line_geom geometry, linedata_code integer, linedata_identifier char(13), linedata_name char(70), linedata_geom geometry, pointdata_gid integer, pointdata_oid_ varchar(254), pointdata_os_toid varchar(254), pointdata_easting numeric, pointdata_northing numeric, pointdata_geom geometry);

--e.g. SELECT * FROM ni_nearest_point_to_line_segment('data_national_grid_gas_pipeline_feeder', 'geom', 'gid', 'linedata_', 'OS_WaterPumpingStations', 'geom', 'gid', 'pointdata_', 500, 500, 'test_output', false)f( gid integer, geom geometry, length double precision, buffer_geom geometry);


--for each section of line supplied, split the line by the interval using the following function:
	--SELECT ST_Segmentize(geom, sub_sep_distance)
	--store the collection of segmentized geometries in to a temporary table
	--loop around every newly created segment and create a buffer 

--do we potentially need another function that does the same operation but without a set buffer size, and just finds "the nearest"	
	
CREATE OR REPLACE FUNCTION ni_nearest_point_to_line_segment(varchar, varchar, varchar, varchar, varchar, varchar, varchar, varchar, float, float, varchar, boolean)
...
COST 100;
ALTER FUNCTION ni_nearest_point_to_line_segment(varchar, varchar, varchar, varchar, varchar, varchar, varchar, varchar, float, float, varchar, boolean) OWNER TO postgres;