--a function to remove the duplicate edges and return a table
--inputs would need to be:
--$1 - edge_table_name
--$2 - edge_join_key_column_name
--$3 - join_table_name
--$4 - output_table_name

CREATE OR REPLACE FUNCTION ni_data_proc_detect_and_combine_duplicate_edges(varchar, varchar, varchar, integer, varchar, varchar)  
RETURNS varchar AS
$BODY$
DECLARE
...
BEGIN
	...
	FOR loop_record IN EXECUTE 'SELECT dup_count_1.'||quote_ident(edge_join_key_column_name)||' AS gid FROM '||quote_ident(duplicate_table_name_count_1)||' AS dup_count_1' LOOP
		...
		EXECUTE 'INSERT INTO '||quote_ident(temp_unique_table_name)||'(gid_copy, final_geom) VALUES ('||quote_literal(loop_record.gid)||',ST_GeomFromText('||quote_literal(final_geom)||', 27700))';
	END LOOP;
...
COST 100;
ALTER FUNCTION ni_data_proc_detect_and_combine_duplicate_edges(varchar, varchar, varchar, integer, varchar, varchar) OWNER TO postgres; 	
