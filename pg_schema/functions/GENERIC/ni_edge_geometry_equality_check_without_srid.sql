-- Function: ni_edge_geometry_equality_check(character varying, character varying)

-- DROP FUNCTION ni_edge_geometry_equality_check(character varying, character varying);

CREATE OR REPLACE FUNCTION ni_edge_geometry_equality_check(character varying, character varying)
  RETURNS integer AS
$BODY$ 
DECLARE
...
BEGIN
    ...
    EXECUTE 'SELECT edge_geometry_table.GeomID FROM '||quote_ident(edge_geometry_table_name)||' AS edge_geometry_table WHERE ST_Equals(ST_GeomFromText('||quote_literal(geometry_to_compare)||'), edge_geometry_table.geom)' INTO matched_geom_id;
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION ni_edge_geometry_equality_check(character varying, character varying) OWNER TO postgres;
