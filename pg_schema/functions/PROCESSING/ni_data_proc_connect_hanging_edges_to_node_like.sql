...
BEGIN
	...
	FOR node_record IN EXECUTE 'SELECT ST_AsText('||quote_ident(node_geometry_column_name)||') AS geom, node_table.'||quote_ident(node_table_attribute)||' as node_table_attribute_value, node_table.* FROM '||quote_ident(node_table_name)||' AS node_table ORDER BY '||quote_ident(node_join_key_column_name)||' ASC' LOOP				
		...
		--RAISE NOTICE 'edge_geometry_column_name: %', edge_geometry_column_name;
		...
		--RAISE NOTICE 'edge_sql: %', edge_sql;
...
$BODY$
  ...
  COST 100
  ROWS 1000;
ALTER FUNCTION ni_data_proc_connect_hanging_edges_to_node_like(character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, integer, character varying, boolean) OWNER TO postgres;
