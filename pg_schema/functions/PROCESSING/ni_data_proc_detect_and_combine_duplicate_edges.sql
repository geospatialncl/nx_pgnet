...
BEGIN
	...
	FOR loop_record IN EXECUTE 'SELECT (dup_count_1.'||quote_ident(edge_join_key_column_name)||'::'||edge_join_key_column_type||') AS gid FROM '||quote_ident(duplicate_table_name_count_1)||' AS dup_count_1 ORDER BY gid ASC' LOOP
		...
		EXECUTE 'SELECT ST_AsText(ST_Union(ARRAY(SELECT ST_SimplifyPreserveTopology(additional_combined_geom, 0.01) FROM '||quote_ident(join_table_name)||' WHERE '||quote_ident(edge_join_key_column_name)||' = '||quote_literal(loop_record.gid)||' ORDER BY '||quote_ident(edge_join_key_column_name)||')))' INTO union_additional_combined_geom;
		...
		IF num_geom > 1 THEN
			...
			EXECUTE 'SELECT ST_AsText(ST_Union(ARRAY(SELECT ST_SimplifyPreserveTopology('||quote_ident(edge_geometry_column_name)||', 0.01) FROM '||quote_ident(join_table_name)||' WHERE '||quote_ident(edge_join_key_column_name)||' = '||quote_literal(loop_record.gid)||' ORDER BY '||quote_ident(edge_join_key_column_name)||')))' INTO orig_geom;
		ELSE		
		...
		IF union_additional_combined_geom IS NOT NULL THEN
			...
			EXECUTE 'SELECT ST_AsText(ST_Union(ST_SimplifyPreserveTopology(ST_GeomFromText('||quote_literal(union_additional_combined_geom)||', 27700), 0.01), ST_SimplifyPreserveTopology(ST_GeomFromText('||quote_literal(orig_geom)||', 27700), 0.01)))' INTO final_geom;
		ELSE
